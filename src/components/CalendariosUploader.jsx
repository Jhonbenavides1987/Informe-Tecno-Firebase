...
// --- FINAL VERSION - Replicating DurableUploader's Logic ---
...
const CalendariosUploader = () => {
    ...
    // 1. File Parsing Logic (from DurableUploader)
    ...
    // 2. Duplicate Removal Logic (adapted for Calendarios)
    const uniqueRowsToUpload = useMemo(() => {
        const idColumn = 'ID_PDV'; // The key column for identifying duplicates
        const pdvIndex = headers.indexOf(idColumn);
        
        if (pdvIndex === -1) {
            return rows.filter(row => {
                return Object.values(row).some(cell => String(cell).toLowerCase().includes(searchTerm.toLowerCase()));
            });
        }
        ...
        const dataToFilter = searchTerm ? rows.filter(row => {
             return Object.values(row).some(cell => String(cell).toLowerCase().includes(searchTerm.toLowerCase()));
        }) : rows;
    ...
    // 3. Upload Logic (from DurableUploader)
