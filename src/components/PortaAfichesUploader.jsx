...
const PortaAfichesUploader = () => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            for (let i = 0; i < dataToUpload.length; i += batchSize) {
                ...
                chunk.forEach(item => {
                    const docRef = doc(collection(db, "ImplementacionPortaAfiches"));
    ...
    const handleDeleteAll = async () => {
        ...
        try {
            const [deletedImplementacion, deletedProgreso] = await Promise.all([
                deleteCollection('ImplementacionPortaAfiches'),
                deleteCollection('ProgresoPortaAfiches')
            ]);
    ...
    return (
        <Paper elevation={3} sx={{ p: 4 }}>
            <Typography variant="h6" gutterBottom>Cargar Archivo de Implementación de Porta Afiches</Typography>
...
export default PortaAfichesUploader;
