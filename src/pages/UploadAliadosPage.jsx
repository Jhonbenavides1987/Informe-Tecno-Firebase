...
const UploadAliadosPage = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      for (let i = 0; i < dataToUpload.length; i += batchSize) {
          ...
          chunk.forEach(item => {
              const docRef = doc(collection(db, "Implementacion Aliados"));
              batch.set(docRef, item);
          });
          await batch.commit();
          setUploadProgress(((i + chunk.length) / dataToUpload.length) * 100);
      }
      setUploadSuccess(`¡Éxito! Se cargaron ${dataToUpload.length} registros únicos de Aliados.`);
      resetImplementationState();
    } catch (err) {
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 2, mb: 4 }}>
      <Typography variant="h4" gutterBottom align="center">Carga de Datos de Aliados</Typography>
      <Grid container spacing={2}>
        <Grid item xs={12} md={3}>
          <Paper elevation={3} sx={{ p: 2, position: 'sticky', top: 20 }}>
            ...
            <Typography variant="body1" sx={{fontWeight: 'bold'}}>Archivo de Implementaciones (Aliados)</Typography>
            {!implementationFile ? (
          ...
          <Paper elevation={3} sx={{ p: 2, width: '100%' }}>
            ...
            {fileHeaders.length > 0 && (
              <FormControl fullWidth sx={{ my: 2 }} size="small">
                <InputLabel id="pdv-id-label-aliados">Columna de ID de PDV (para validación)</InputLabel>
                <Select labelId="pdv-id-label-aliados" value={selectedPdvIdColumn} label="Columna de ID de PDV (para validación)" onChange={e => setSelectedPdvIdColumn(e.target.value)}>
                  {fileHeaders.map(h => <MenuItem key={`pdv-${h}`} value={h}>{h}</MenuItem>)}
                </Select>
              </FormControl>
            )}
            <Typography variant="h6" mt={2}>3. Filtrar y Cargar (Aliados)</Typography>
            <Box sx={{ my: 2 }}>
...
export default UploadAliadosPage;
