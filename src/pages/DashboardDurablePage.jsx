...
const aggregateData = (data, key) => {
  ...
  const grouped = data.reduce((acc, item) => {
    ...
    acc[groupName].total++;
    if (item.implementado_durable) { // <-- Durable field
            ...
            <TableBody>
              {finalDetailedData.map(pdv => (
                <TableRow key={pdv.id} sx={{ backgroundColor: circuitColorMap[pdv.CIRCUITO || 'Sin Asignar'], '&:hover': { backgroundColor: 'action.hover' } }}>
                  ...
                  <TableCell align="center">
                    <Chip label={pdv.implementado_durable ? '100%' : '0%'} sx={pdv.implementado_durable ? getStatusStyle(100) : getStatusStyle(0)} size="small"/>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
