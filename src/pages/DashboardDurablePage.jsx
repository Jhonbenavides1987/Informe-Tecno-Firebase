...
const aggregateData = (data, key) => {
  ...
  const grouped = data.reduce((acc, item) => {
    ...
    acc[groupName].total++;
    if (item.implementado_durable) { // <-- CAMBIO CLAVE: Usar campo de Durable
...
// --- Main Dashboard Durable Page Component ---
const DashboardDurablePage = () => {
  ...
  const finalDetailedData = useMemo(() => {
    ...
    if (view === 'implemented') data = data.filter(pdv => pdv.implementado_durable);
    if (view === 'unimplemented') data = data.filter(pdv => !pdv.implementado_durable);
  ...
  const mainMetrics = useMemo(() => {
      ...
      const implementados = filteredPdv.filter(pdv => pdv.implementado_durable).length;
  ...
  const handleExport = () => {
    const dataToExport = finalDetailedData.map(pdv => ({
      ...
      'ESTADO': pdv.implementado_durable ? 'Implementado' : 'No Implementado'
    }));

    const worksheet = XLSX.utils.json_to_sheet(dataToExport);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Detalle Pdv Filtrado Durable");
    XLSX.writeFile(workbook, "DetallePdvDurable.xlsx");
  };
  ...
  return (
    <Box sx={{ p: { xs: 1, sm: 2, md: 3 }, maxWidth: 1400, margin: 'auto' }}>
      <Typography variant="h4" gutterBottom sx={{ mb: 3, textAlign: 'center' }}>Dashboard de Implementación (Durable)</Typography>
      ...
      <Paper elevation={2} sx={{ p: 2, mt: 4, overflowX: 'auto' }}>
        <Box sx={{display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, justifyContent: 'space-between', alignItems: 'center', mb: 2, gap: 2}}>
          <Typography variant="h6">Detalle de Puntos de Venta (Durable)</Typography>
            ...
            <TableBody>
              {finalDetailedData.map(pdv => (
                ...
                >
                  ...
                  <TableCell align="center">
                    <Chip 
                      label={pdv.implementado_durable ? '100%' : '0%'} 
                      sx={pdv.implementado_durable ? getStatusStyle(100) : getStatusStyle(0)} 
                      size="small"
                    />
                  </TableCell>
                </TableRow>
...
export default DashboardDurablePage;
