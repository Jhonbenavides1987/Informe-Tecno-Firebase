...
const aggregateData = (data, key) => {
  ...
  const grouped = data.reduce((acc, item) => {
    ...
    acc[groupName].total++;
    if (item.implementado_activacion) { // <-- CAMBIO CLAVE: Usar campo de Activacion
...
// --- Main Dashboard Activacion Page Component ---
const DashboardActivacionPage = () => {
  ...
  const finalDetailedData = useMemo(() => {
    ...
    if (view === 'implemented') data = data.filter(pdv => pdv.implementado_activacion);
    if (view === 'unimplemented') data = data.filter(pdv => !pdv.implementado_activacion);
  ...
  const mainMetrics = useMemo(() => {
      ...
      const implementados = filteredPdv.filter(pdv => pdv.implementado_activacion).length;
  ...
  const handleExport = () => {
    const dataToExport = finalDetailedData.map(pdv => ({
      ...
      'ESTADO': pdv.implementado_activacion ? 'Implementado' : 'No Implementado'
    }));

    const worksheet = XLSX.utils.json_to_sheet(dataToExport);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Detalle Pdv Filtrado Activacion");
    XLSX.writeFile(workbook, "DetallePdvActivacion.xlsx");
  };
  ...
  return (
    <Box sx={{ p: { xs: 1, sm: 2, md: 3 }, maxWidth: 1400, margin: 'auto' }}>
      <Typography variant="h4" gutterBottom sx={{ mb: 3, textAlign: 'center' }}>Dashboard de Implementación (Activación)</Typography>
      ...
      <Paper elevation={2} sx={{ p: 2, mt: 4, overflowX: 'auto' }}>
        <Box sx={{display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, justifyContent: 'space-between', alignItems: 'center', mb: 2, gap: 2}}>
          <Typography variant="h6">Detalle de Puntos de Venta (Activación)</Typography>
            ...
            <TableBody>
              {finalDetailedData.map(pdv => (
                ...
                >
                  ...
                  <TableCell align="center">
                    <Chip 
                      label={pdv.implementado_activacion ? '100%' : '0%'} 
                      sx={pdv.implementado_activacion ? getStatusStyle(100) : getStatusStyle(0)} 
                      size="small"
                    />
                  </TableCell>
                </TableRow>
...
export default DashboardActivacionPage;
