...
const aggregateData = (data, key) => {
  ...
  const grouped = data.reduce((acc, item) => {
    ...
    acc[groupName].total++;
    if (item.implementado_activacion) { // <-- Use Activacion field
            ...
            <TableBody>
              {finalDetailedData.map(pdv => (
                <TableRow key={pdv.id} sx={{ backgroundColor: circuitColorMap[pdv.CIRCUITO || 'Sin Asignar'], '&:hover': { backgroundColor: 'action.hover' } }}>
                  ...
                  <TableCell align="center">
                    <Chip label={pdv.implementado_activacion ? '100%' : '0%'} sx={pdv.implementado_activacion ? getStatusStyle(100) : getStatusStyle(0)} size="small"/>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
