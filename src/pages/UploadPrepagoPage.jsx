...
const UploadPrepagoPage = ({ user }) => {
  ...
  const handleDeleteCollection = async (collectionName) => {
    ...
    try {
        ...
        if (collectionName === 'Implementacion Prepago') resetImplementationState();
  ...
  const fetchMasterPdvIds = async () => {
    try {
      ...
      setUploadError("Error al cargar la base maestra de PDV de Prepago.");
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      for (let i = 0; i < dataToUpload.length; i += batchSize) {
          ...
          chunk.forEach(item => {
              const docRef = doc(collection(db, "Implementacion Prepago"));
              batch.set(docRef, item);
          });
          await batch.commit();
          setUploadProgress(((i + chunk.length) / dataToUpload.length) * 100);
      }
      setUploadSuccess(`¡Éxito! Se cargaron ${dataToUpload.length} registros únicos de Prepago.`);
      resetImplementationState();
    } catch (err) {
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 2, mb: 4 }}>
      <Typography variant="h4" gutterBottom align="center">Carga de Datos de Prepago</Typography>
      <Grid container spacing={2}>
        <Grid item xs={12} md={3}>
          <Paper elevation={3} sx={{ p: 2, position: 'sticky', top: 20 }}>
            <Typography variant="h6" gutterBottom>1. Cargar Archivos (Prepago)</Typography>
            {deleteMessage.text && <Alert severity={deleteMessage.type} sx={{mb: 2}}>{deleteMessage.text}</Alert>}
            
            <Typography variant="body1" sx={{fontWeight: 'bold'}}>Base Maestra (Prepago)</Typography>
            <DataUploader key={`uploader-prepago-${uploaderKey}`} collectionName="Base Prepago" onUploadSuccess={fetchMasterPdvIds}/>
            ...
            <Tooltip title={!user ? "Debes iniciar sesión para borrar datos" : ""}>
              <span>
                <Button fullWidth variant="contained" color="error" size="small" onClick={() => handleDeleteCollection('Base Prepago')} sx={{ mt: 1 }} disabled={isDeleting.base || !user} startIcon={<DeleteForever />}>
                  {isDeleting.base ? 'Borrando...' : 'Borrar Base Prepago'}
                </Button>
              </span>
            </Tooltip>
            ...
            <Typography variant="body1" sx={{fontWeight: 'bold'}}>Archivo de Implementaciones (Prepago)</Typography>
            {!implementationFile ? (
            ...
            <Tooltip title={!user ? "Debes iniciar sesión para borrar datos" : ""}>
              <span>
                <Button fullWidth variant="contained" color="error" size="small" onClick={() => handleDeleteCollection('Implementacion Prepago')} sx={{ mt: 1 }} disabled={isDeleting.impl || !user} startIcon={<DeleteForever />}>
        ...
        <Grid item xs={12} md={9}>
          <Paper elevation={3} sx={{ p: 2, width: '100%' }}>
             <Alert severity="warning" sx={{mb: 2}}>
              ...
              Se han cargado <strong>{masterPdvIds.size}</strong> IDs únicos desde la <strong>Base Prepago</strong> en memoria para la validación.
              Si este número es 0, por favor, asegúrate de haber subido el archivo correcto de 'Base Prepago'.
            </Alert>

            <Typography variant="h6">2. Configurar y Previsualizar (Prepago)</Typography>
            {fileHeaders.length > 0 && (
              <FormControl fullWidth sx={{ my: 2 }} size="small">
                <InputLabel id="pdv-id-label-prepago">Columna de ID de PDV (para validación)</InputLabel>
                <Select labelId="pdv-id-label-prepago" value={selectedPdvIdColumn} label="Columna de ID de PDV (para validación)" onChange={e => setSelectedPdvIdColumn(e.target.value)}>
                  {fileHeaders.map(h => <MenuItem key={`pdv-${h}`} value={h}>{h}</MenuItem>)}
                </Select>
              </FormControl>
            )}
            <Typography variant="h6" mt={2}>3. Filtrar y Cargar (Prepago)</Typography>
            <Box sx={{ my: 2 }}>
...
export default UploadPrepagoPage;
