...
const UploadDurablePage = ({ user }) => {
  ...
  const handleDeleteCollection = async (collectionName) => {
    ...
    try {
        ...
        if (collectionName === 'Implementacion Durable') resetImplementationState();
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      for (let i = 0; i < dataToUpload.length; i += batchSize) {
          ...
          chunk.forEach(item => {
              const docRef = doc(collection(db, "Implementacion Durable"));
              batch.set(docRef, item);
          });
          await batch.commit();
          setUploadProgress(((i + chunk.length) / dataToUpload.length) * 100);
      }
      setUploadSuccess(`¡Éxito! Se cargaron ${dataToUpload.length} registros únicos de Durable.`);
      resetImplementationState();
    } catch (err) {
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 2, mb: 4 }}>
      <Typography variant="h4" gutterBottom align="center">Carga de Datos de Durable</Typography>
      <Grid container spacing={2}>
        <Grid item xs={12} md={3}>
          <Paper elevation={3} sx={{ p: 2, position: 'sticky', top: 20 }}>
            <Typography variant="h6" gutterBottom>1. Cargar Archivos (Durable)</Typography>
            ...
            <DataUploader key={`uploader-durable-${uploaderKey}`} collectionName="Base Prepago" onUploadSuccess={fetchMasterPdvIds}/>
            ...
            <Typography variant="body1" sx={{fontWeight: 'bold'}}>Archivo de Implementaciones (Durable)</Typography>
            {!implementationFile ? (
            ...
            <Tooltip title={!user ? "Debes iniciar sesión para borrar datos" : ""}>
              <span>
                <Button fullWidth variant="contained" color="error" size="small" onClick={() => handleDeleteCollection('Implementacion Durable')} sx={{ mt: 1 }} disabled={isDeleting.impl || !user} startIcon={<DeleteForever />}>
        ...
        <Grid item xs={12} md={9}>
          <Paper elevation={3} sx={{ p: 2, width: '100%' }}>
            ...
            <Typography variant="h6">2. Configurar y Previsualizar (Durable)</Typography>
            {fileHeaders.length > 0 && (
              <FormControl fullWidth sx={{ my: 2 }} size="small">
                <InputLabel id="pdv-id-label-durable">Columna de ID de PDV (para validación)</InputLabel>
                <Select labelId="pdv-id-label-durable" value={selectedPdvIdColumn} label="Columna de ID de PDV (para validación)" onChange={e => setSelectedPdvIdColumn(e.target.value)}>
                  {fileHeaders.map(h => <MenuItem key={`pdv-${h}`} value={h}>{h}</MenuItem>)}
                </Select>
              </FormControl>
            )}
            <Typography variant="h6" mt={2}>3. Filtrar y Cargar (Durable)</Typography>
            <Box sx={{ my: 2 }}>
...
export default UploadDurablePage;
